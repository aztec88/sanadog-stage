(function ($) {
    ...
    var ella = {
        ...
        closeAnnouncementBar: function () {
            if ($('.home-announcement-sections').length) {
                var announcementEml = $('.home-announcement'),
                    closeannouncementElm = announcementEml.find('[data-close-header-top]');
                ...
                closeannouncementElm.off('click.closeAnnouncement').on('click.closeAnnouncement', function (e) {
                    e.preventDefault();
                    e.stopPropagation();
                    announcementEml.remove();
        ...
        closeAllOnMobile: function () {
            body.off('click.close', wrapperOverlaySlt).on('click.close', wrapperOverlaySlt, function (e) {
                ...
                html.removeClass('search-dropdown-open');
            ...
            var load = function(){
                element.each(function(){
                    ...
                    if (!self.hasClass('ajax-loaded')) {
                        ...
                        if (self.hasClass('suppermarket-featured-products')){
                          var view_json_product = 'json-feature-suppermarket';
                        }else{
                          var view_json_product = 'list_product';
                        }
                        ...
                        if (top < window.innerHeight + 10) {
                            $.ajax({
                                ...
                                complete: function () {
                                    if (self.attr('data-featured-products-section') == undefined && self.attr('data-featured-products-section-2') == undefined) {
                                        return;
                                    }
                                    if (layout == 'carousel') {
                                        ...
                                        if(productGrid.not('.slick-initialized')) {
                                            productGrid.slick({
                                                ...
                                                get dots() {
                                                    if((self.hasClass('has-banner')) || (window.layout_style == 'layout_style_suppermarket') || (window.layout_style == 'layout_style_surfup') || (window.layout_home == 'layout_home_12')){
                                                        return this.dots = true;
                                                    }else {
                                                        return this.dots = false;
                                                    };
        ...
        initDropdownCartMobile: function () {
            var headerMb = $('.header-mb, [data-cart-header-parallax], [data-cart-header-02], [data-cart-header-04], [data-cart-header-05], [data-cart-header-supermarket], [data-cart-sidebar], .halo-sticky-toolbar-mobile'),
        ...
        removeItemDropdownCart: function (cart) {
            ...
            btnRemove.off('click.removeCartItem').on('click.removeCartItem', function (e) {
                ...
                productKey = $(this).parents('.item').data("key");
            ...
            btnUpdate.off('change.addCartItem').on('change.addCartItem', function (e) {
                ...
                e.stopPropagation();
                var btn = $(this);
                var productKey = btn.closest("[data-key]").data("key");
                var quantity = parseInt(btn.val());
                $(this).parents('.item').addClass('cur--change');
                ella.doAjaxUpdatePopupCart(quantity, productKey);
            });
        },
        ...
        initColorSwatchGrid: function () {
            ...
            body.off('click.toggleClass').on('click.toggleClass', itemSwatchSlt, function () {
                ...
                if (window.product_variant_name == true) {
                    if(window.layout_style == 'layout_style_surfup') {
                        ...
                        (productItemElm.find('.product-title').hasClass('change-text')) ? 
                        productItemElm.find('.product-title').find('[data-change-title]').html('<span class="line"> - </span>' + title) :
                        productItemElm.find('.product-title').addClass('change-text').append('<span data-change-title><span class="line"> - </span></span>' + title + '</span>');
                    }
                }
...
//                     productItemElm.find('[data-value="' + title + '"]').find('label').trigger('click');
        ...
        initInfiniteScrolling: function () {
            ...
            if (window.collection_pagination_type == "scroll") {
                $(window).scroll(function () {
                    if (ella.isAjaxLoading) return;
        ...
        viewModeLayout: function () {
            var viewMode = $('[data-view-as]'),
                ...
                strClass = 'col-12 col-6 col-md-4 col-lg-3 col4 col5',
                ...
                default:
                    switch (col) {
                        ...
                        case 4:
                            ...
                            gridItem.removeClass(strClass).addClass('col-6 col-md-4 col-lg-3 col4');
                            break;
        ...
        changeQuantityAddToCart: function () {
            ...
            inputQ.on('change', function(){
                ...
                var productItem = self.parents('.cart-product-item'),
                    ...
                    productKey = $(productItem).data('key');
                ...
                if (line < val) {
                    ...
                    ella.doAjaxUpdateCart(productKey, val, line, Price, productItem);
                }
            });
        ...
        doAjaxUpdateCart: function(productKey, quantity, cartButton, Price, productItem){
        ...
        doAjaxUpdatePopupCart: function (quantity, productKey) {
            $.ajax({
                type: 'POST',
                url: '/cart/change.js',
                data: {
        ...
        initChangeQuantityButtonEvent: function () {
            ...
            doc.off('click.updateCart').on('click.updateCart', buttonSlt, function (e) {
                ...
                var productKey = btn.closest("[data-product-id]").data("key");
            ...
            doc.on('change', inputQ, function () {
                var self = $(this);
                var productItem = self.parents('.product'),
                    line = self.data('line'),
                    val = self.val(),
                    productId = $(productItem).data('item-id'),
                    productKey = $(productItem).data('key');
        ...
        initQuantityInputChangeEvent: function () {
            ...
            doc.on('change', quantityIptSlt, function () {
                var id = $(this).closest("[data-product-id]").data("product-id"),
                    ...
                    productKey = $(this).closest("[data-product-id]").data("key");
        ...
        removeCartItem: function () {
            ...
            doc.on('click', removeSlt, function (e) {
                ...
                var id = $(this).closest("[data-product-id]").data("product-id");
                var productKey = $(this).closest("[data-product-id]").data("key");

                ella.doAjaxUpdatePopupCart(0, productKey);
            });
        },
        ...
        initStickyForProductFullWidth: function() {
            ...
            initSticky = function() {
                if (stickyElm1.outerHeight() < stickyElm2.outerHeight() || stickyElm1.outerHeight() < stickyElm3.outerHeight()) {
        ...
        openSearchForm: function () {
            var iconSearchSlt = '[data-search-mobile-toggle]',
                ...
                closeSearch = $('.search-open .wrapper-header .close-search');
            ...
            doc.off('click.toggleSearch', iconSearchSlt).on('click.toggleSearch', iconSearchSlt, function(e) {
                ...
                if (!self.parents(".header-search").find('.quickSearchResultsWrap .header-search__product .products-grid .product-item').length) {
                    $.ajax({
                        type: "get",
                        url: window.router + '/collections/' + url,
                        cache: false,
            ...
            doc.off('click.hideSearch').on('click.hideSearch', function (e) {
                ...
                var iconSearchSlt = $('[data-search-mobile-toggle]');
                var slideshow = $('.home-slideshow .slide-action');
                var searchBottom = $('.halo-sticky-toolbar-mobile');
                var searchSidebar = $('#search-form-mobile');

                if (!searchForm.is(e.target) && !iconSearchSlt.is(e.target) && !slideshow.has(e.target).length && !searchForm.has(e.target).length && !searchProduct.has(e.target).length && !searchTrending.has(e.target).length && !overlay.has(e.target).length && !searchBottom.is(e.target) && !searchSidebar.is(e.target) && !searchSidebar.has(e.target).length) {
                    html.removeClass('search-open');
                    $('.close-search-style-3').hide();
                    html.removeClass('search-dropdown-open');
            ...
            $('.close-search-style-3, [data-search-close-sidebar]').off('click.hideQuickSearchStyle3').on('click.hideQuickSearchStyle3', function (e) {
                e.preventDefault();
                e.stopPropagation();
                $('.quickSearchResultsWrap').hide();
                $('.close-search-style-3').hide();
        ...
        _multiHomepage: function (){
            ...
            var pageCurrent = $('body').data('page-handle');
        ...
        initCustomInfo: function() {
            doc.on('click', '[data-custom-information]', (event) => {
                event.preventDefault();
            ...
            doc.on('click', '[data-close-custom-information]', (event) => {
                event.preventDefault();
                event.stopPropagation();
            ...
            doc.on('click', '.wrapper-overlay', (event) => {
                event.preventDefault();
                event.stopPropagation();
...
document.addEventListener('easytabs_page_inited', function(){
  	...
  	divBg.classList.add('easyBg');
	document.querySelector('.easytabs-container').append(divBg)
});
